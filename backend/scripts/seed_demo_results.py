...
SPECS = [
    ...
    ("home", 2.30, True, "win", 1.30),
    ("draw", 3.20, True, "win", 2.20),
    ("under", 1.85, False, "loss", -1.0),
    ("home", 2.05, True, "win", 1.05),
    ("away", 2.40, False, "loss", -1.0),
    ("over", 2.00, True, "win", 1.00),
    ("home", 1.95, True, "win", 0.95),
    ("draw", 3.10, False, "loss", -1.0),
    ("away", 2.20, True, "win", 1.20),
]


async def _wipe(session) -> None:
    lg = (
        await session.execute(select(League).where(League.sport_key == SPORT_KEY))
    ).scalar_one_or_none()
    if lg is None:
        return
    fids = (
        (await session.execute(select(Fixture.id).where(Fixture.league_id == lg.id)))
        .scalars()
        .all()
    )
    sids = (
        (await session.execute(select(Signal.id).where(Signal.fixture_id.in_(fids))))
        .scalars()
        .all()
    )
    if sids:
    ...
    async with Session() as s:
        ...
        for i, (sel, odds, beat, result, pnl) in enumerate(SPECS):
            ...
            sig = Signal(
                ...
                kelly_frac=0.02,
                ttl_sec=1800,
                dedup_hash=f"demo_{i}",
                status="settled",
                created_at=now - dt.timedelta(hours=(len(SPECS) - i) * 6),
            )
            s.add(sig)
            await s.flush()
            s.add(
