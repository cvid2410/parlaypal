...
async def signal_context(session, sig: Signal) -> SignalCopyContext | None:
    ...
    fixture = (
        await session.execute(select(Fixture).where(Fixture.id == sig.fixture_id))
    ).scalar_one_or_none()
    if fixture is None:
        return None
    market = (
        await session.execute(select(Market).where(Market.id == sig.market_id))
    ).scalar_one_or_none()
    league = (
        await session.execute(select(League).where(League.id == fixture.league_id))
    ).scalar_one_or_none()
    home = (
        await session.execute(select(Team).where(Team.id == fixture.home_id))
    ).scalar_one_or_none()
    away = (
        await session.execute(select(Team).where(Team.id == fixture.away_id))
    ).scalar_one_or_none()
    ...
    return SignalCopyContext(
        ...
        book=sig.book,
        offered_decimal=sig.offered_odds,
        fair_prob=sig.fair_prob,
        edge_pct=sig.edge_pct,
        kelly_frac=sig.kelly_frac,
        legs=legs,
        window=window,
    )
