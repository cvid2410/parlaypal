...
async def test_discovery_registers_and_toggles():
    ...
    async with Session() as s:
        ...
        enabled_keys = (await s.execute(
            select(League.sport_key).where(League.ingest_enabled.is_(True))
        )).scalars().all()
        s.add(League(name="Stale", country="X", sport_key=stale_key, is_soft=True,
                     ingest_enabled=True))
        await s.commit()
    ...
    try:
        ...
        async with Session() as s:
            new_lg = (await s.execute(select(League).where(League.sport_key == new_key))).scalar_one()
            assert new_lg.is_soft is True and new_lg.ingest_enabled is True
            assert new_lg.country == "Wonderland"
            stale = (await s.execute(select(League).where(League.sport_key == stale_key))).scalar_one()
